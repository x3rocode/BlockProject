...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
        ...
        public Vector3 movedMousePos;
        public float rotValue; //저번처럼 이런걸하나  만드럿단다 ㅇㅋ??웅
        public  bool CheckMovedMousePos()
        {
            if (movedMousePos.x >= 1) return true;
            if (movedMousePos.x <= -1) return true;

            if (movedMousePos.y >= 1) return true;
            if (movedMousePos.y <= -1) return true;

            if (movedMousePos.z >= 1) return true;
            if (movedMousePos.z <= -1) return true;
            return false;
        }
        public Vector3 GetMoveAxis()
        {
            if (movedMousePos.x >= 1) { return new Vector3(1, 0, 0); }
            if (movedMousePos.x <= -1) { return new Vector3(-1, 0, 0); }

            if (movedMousePos.y >= 1) { return new Vector3(0, 1, 0); }
            if (movedMousePos.y <= -1) { return new Vector3(0, -1, 0); }


            if (movedMousePos.z >= 1) { return new Vector3(0, 0, 1); }
            if (movedMousePos.z <= -1) { return new Vector3(0, 0, -1); }

            return Vector3.zero;
        }
      
        public bool CheckMousePosRotate()
        {
            if (movedMousePos.x >= 30) return true;
            if (movedMousePos.x <= -30) return true;

            if (movedMousePos.y >= 30) return true;
            if (movedMousePos.y <= -30) return true;

            if (movedMousePos.z >= 30) return true;
            if (movedMousePos.z <= -30) return true;
            return false;
        }
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						if(hasTranslatingAxisPlane)
						{
                            ...
                            movedMousePos += movement; 
                        }

                        if (CheckMovedMousePos())
                        { 
                            for (int i = 0; i < targetRootsOrdered.Count; i++)
                            {
